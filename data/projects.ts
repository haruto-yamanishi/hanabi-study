...
export const projects: Project[] = [
  {
    id: 'project-model', title: '01 · 計算とコードでロボットを予測する', hours: 10,
    ...
    brief: '質量4 kgの台車を静止から1 m/s²で2秒加速、その後2秒等速にする。位置の理論値は2秒で2 m、4秒で6 m。速度測定へ+0.02 m/sのバイアスを加え、積分誤差を検証する。刻み0.1秒と0.02秒を比較する。',
  ...
  {
    id: 'project-mechanism', title: '02 · 製造できるIntakeを設計する', hours: 20,
    ...
    brief: '学習用のローラ直径60 mm、幅250 mm、必要表面速度1.5 m/s、接線負荷20 Nで設計する。要求出力は約477 rpm、トルク0.6 N·m。設計荷重はまず2倍で検討するが、この倍率だけで安全を保証しない。モータ仕様の負荷時動作点を使う。',
    ...
    rubric: [{id:'sizing',text:'477 rpmと0.6 N·mの算出過程があり、効率・始動・負荷変動も評価している。'},{id:'drawing',text:'CAD、図面、BOMの版が一致し、材料と重要公差がある。'},{id:'loads',text:'軸・支持・締結の荷重と変形を説明し、許容値の根拠がある。'},{id:'assembly',text:'組立順序、工具空間、交換時間、製作または仮想組立の結果を残している。'}],
  },
  {
    id: 'project-electrical', title: '03 · 配電とセンサの立ち上げ', hours: 14,
    ...
    brief: 'モータ2系統と位置センサ、CAN機器、制御計算機の電装設計を作る。例題の電源E=12.6 V、内部抵抗0.02 Ω、合計60 Aでは11.4 Vとなる。これは規則値ではない。採用機器と当該シーズンの線径・保護・接地規則を別に確認する。',
    procedure: ['電源から各負荷までの系統図、線径・端子・保護・CAN ID・終端・IPの表を作る。規則と仕様書の参照を記す。', '無通電で極性、導通、圧着、固定、終端を検査する。抵抗測定を通電中に行わない。', 'シミュレーション、次に可能なら監督下の低出力実習で、正指令→正位置、1回転換算、通信状態を確認する。', '断線・終端不足・接触抵抗増加を紙上またはシミュレーションで注入し、測定点と予想値を照合する。危険な短絡を実機で作らない。'],
  ...
  },
  {
    ...
    goal: 'FF+PIDと軌道制約を設計し、負荷が変わっても要求を検証する。',
    equipment: 'PCと下の制御実験室、CSV解析。実機では落下防止・可動域・停止手段を監督者と確保。',
    brief: '4 kgの台車を0→0.6 mへ移動する。学習用要求は速度≤0.8 m/s、加速度≤1 m/s²、終点誤差±0.02 mへ3秒以内に整定、0.5秒維持。理想張力は加速時43.2 N（g=9.8）。実機要求はチームが再設定する。',
    procedure: ['質量、半径、減速比から必要トルクと電流を求め、可動域・電圧・電流の制限を定義する。', '台形または三角形プロファイル、原点取得、limit switch、異常停止の状態機械を実装する。', '同定用ログでFFを求め、Pから段階的に調整する。積分飽和とセンサ遅延をシミュレーションで比較する。', '荷重4/5 kgと供給12/10 Vの4条件を各3回評価し、全ログ・最大誤差・整定時間・飽和を残す。要求未達は設計へ戻す。'],
    rubric: [{id:'model',text:'43.2 Nの計算と機構・電装の能力比較があり、保持と加速を分けている。'},{id:'limits',text:'速度・加速度・位置・電流の制限と無効化・原点失敗時の挙動を検証する。'},{id:'response',text:'誤差±0.02 mを0.5秒維持する整定判定を実装し、4条件×3試行を報告する。'},{id:'analysis',text:'FF/P/I/D各項と飽和をログで説明し、未達条件と改善を記録する。'}],
  },
  {
    ...
    brief: 'モジュール位置(±0.3,±0.3) mの4輪車を想定し、1 m角の経路を0.5 m/s以下で周回する。学習用目標は終点位置誤差≤0.10 m、heading誤差≤5°。vision遅延100 msと2秒の欠測区間を追加する。',
    ...
    rubric: [{id:'frames',text:'field/robot/camera変換の往復と±π境界のテストがある。'},{id:'modules',text:'3種類の基本運動と車輪速度上限の共通縮小を検証する。'},{id:'fusion',text:'同じログでodometry単独と融合を比較し、遅延・外れ値・欠測を評価する。'},{id:'path',text:'終点の位置・角度の判定と途中の衝突・誤差・飽和を報告する。'}],
  },
  {
    id: 'project-ai', title: '06 · 現場条件で評価できる認識器', hours: 18,
    ...
    brief: '対象物あり/なしを判定する簡単なモデルを作る。撮影日または環境単位でtrain/validation/testを分け、閾値はvalidationで選ぶ。テスト目標は再現率≥90%、適合率≥90%、推論95パーセンタイル≤100 msとし、未達でもデータを隠さず改善計画を示す。',
  ...
  {
    id: 'project-integration', title: '07 · 全系統の設計レビューと故障診断', hours: 24,
    ...
    brief: '取込→保持→指定位置へ運搬→排出のシーケンスを作る。要求には荷重、所要時間、位置誤差、同時消費電流、通信断時動作を含める。故障はセンサ符号反転、電圧低下、機械摩擦増加、vision遅延をシミュレーションで個別に注入する。',
  ...
  {
    id: 'project-handoff', title: '08 · 卒業制作の引継ぎと口頭レビュー', hours: 10,
