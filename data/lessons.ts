...
export const lessons: Lesson[] = [
  ...
  },
  {
    ...
    steps:[
      ...
      {id:'e1',kind:'example',title:'内積',body:'a=(1,2), b=(3,4)ならa·b=1×3+2×4=11。幾何的には|a||b|cosθ。'},
      {id:'r1',kind:'recall',title:'内積が0なら？',body:'直交ベクトルの内積は0。',prompt:'a·b=0が示す代表的な関係は？',options:['平行','直交','同じ長さ'],answer:1,explanation:'非零ベクトルなら直交。'},
  ...
  {
    id:'l-derivative-motion',skillId:'m-calc-diff',title:'微分 = その瞬間の変化率',summary:'位置→速度→加速度、制御ログを微分の言葉で読む。',estimatedMinutes:24,revision:1,
    steps:[
      {id:'c1',kind:'concept',title:'傾きとしての微分',body:'x(t)の微分dx/dtは速度。速度の微分は加速度。グラフでは接線の傾き。'},
      {id:'e1',kind:'example',title:'x=t²',body:'dx/dt=2t。t=3 sなら速度6 m/s。位置が同じでも傾きが違えば速度は違う。'},
      {id:'r1',kind:'recall',title:'順番を言える？',body:'位置・速度・加速度のつながりを思い出す。',prompt:'速度を時間微分すると？',options:['位置','加速度','仕事'],answer:1,explanation:'dv/dt=a。'},
  ...
  {
    id:'l-forces-fma',skillId:'p-forces',title:'自由物体図からF=maへ',summary:'機構に働く力を漏れなく切り出して、運動を予測する。',estimatedMinutes:25,revision:1,
    steps:[
  ...
  },
  {
    ...
    steps:[
      {id:'c1',kind:'concept',title:'V=IR',body:'電圧は「押す差」、電流は流れる量、抵抗は流れにくさ。P=VIで電力。'},
      {id:'e1',kind:'example',title:'10 V / 5 Ω',body:'I=10/5=2 A。P=10×2=20 W。抵抗で失われる電力はI²Rでも同じ。'},
      {id:'r1',kind:'recall',title:'Voltage drop',body:'connectorやwireにも小さな抵抗がある。',prompt:'同じ抵抗で電流が2倍になるとI²R lossは？',options:['2倍','4倍','変わらない'],answer:1,explanation:'I²に比例するので4倍。'},
  ...
  },
  {
    ...
    steps:[
      ...
      {id:'p1',kind:'practice',title:'Debug order',body:'電源→物理配線→device status→ID/firmware→trafficの順に、観測できる事実を増やしていく。'},
  ...
  },
  {
    ...
    steps:[
      ...
      {id:'e1',kind:'example',title:'12T → 60T',body:'出力側/入力側=60/12=5:1 reduction。速度は約1/5、torqueは理想で約5倍。'},
      {id:'r1',kind:'recall',title:'比を逆にしない',body:'どちらがdriverか、どちらがdrivenかを確認する。',prompt:'12Tが60Tを駆動する減速比は？',options:['1:5','5:1','72:1'],answer:1,explanation:'60/12=5。'},
  ...
  },
  {
    ...
    steps:[
      {id:'c1',kind:'concept',title:'P / I / D',body:'Pは今の誤差、Iは誤差の蓄積、Dは誤差の変化率を見る。まずPだけで挙動を観察し、必要に応じてI/Dを追加する。'},
  ...
  },
  {
    ...
    steps:[
      ...
      {id:'p1',kind:'practice',title:'kS / kV / kA',body:'FRC characterizationでは静摩擦・速度・加速度への必要voltageを近似する。'},
  ...
  },
  {
    ...
    steps:[
      ...
      {id:'e1',kind:'example',title:'+90° rotation',body:'(1,0)を+90°回転すると(0,1)。2D rotation matrixで同じ計算を一般化できる。'},
  ...
  },
  {
    ...
    steps:[
      {id:'c1',kind:'concept',title:'症状→仮説→観測',body:'いきなり部品交換しない。再現条件を固定し、原因候補ごとに「何を測れば否定できるか」を決める。'},
