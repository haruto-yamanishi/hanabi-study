...
export default function Page() {
  ...
  return <div className="app-shell">
    <header className="site-header"><button className="brand" onClick={() => navigate('home')}><span className="brand-mark">花</span><span>Hanabi <b>Study English</b><small>WORDS FOR WHAT COMES NEXT</small></span></button><nav aria-label="Main navigation">{screens.map(item => <button key={item.id} className={screen === item.id ? 'active' : ''} onClick={() => navigate(item.id)}>{item.label}</button>)}</nav></header>
    <main>
      ...
      {snapshot && <>
        {screen === 'home' && <section className="home">
          <div className="hero"><div><p className="eyebrow">YOUR DAILY WORD ROUTINE</p><h1>今日の150を、<br /><em>すぐ始めよう。</em></h1><p className="hero-copy">知らない語から先に。意味が分かる語は、使える語へ。<br />一枚ずつ、テンポよく進めます。</p><button className="primary hero-button" onClick={() => navigate('today')}>今日の150を始める <span>→</span></button><p className="hero-note">今日の予定 {plan?.ids.length ?? 0}語 · 残り {remaining}語</p></div><div className="hero-number"><span>今日あと</span><strong>{remaining}</strong><span>語</span></div></div>
          ...
          <div className="home-bottom"><div><p className="eyebrow">START WHERE YOU ARE</p><h2>最初の診断</h2><p>短いサンプルで、学習を始める難易度帯を探します。診断を飛ばして今日の学習を始めることもできます。</p><button className="text-link" onClick={() => navigate('diagnostic')}>{diagnostic?.finished ? `診断済み · 難易度 ${snapshot.startBand} から` : diagnostic ? '診断を再開する →' : '診断を始める →'}</button></div><div><p className="eyebrow">ONE WORD AT A TIME</p><h2>今日の流れ</h2><p>意味を確認 → 短く入力 → 答え合わせ。○になった語は間隔を空けて確認し、安定したら通常の学習から外します。</p></div></div>
        </section>}
...
function ProgressScreen({ snapshot, counts, onRefresh }: { snapshot: Snapshot; counts: ReturnType<typeof statusCounts>; onRefresh: () => Promise<void> }) {
  ...
  const gainedUsable = snapshot.recentAttempts.filter(attempt => attempt.at >= sevenDaysAgo && attempt.after === 'usable' && attempt.before !== 'usable').length;
