...
export function ControlLab() {
  ...
  return <section className="mt-5 rounded-2xl border border-[#d9dde5] bg-white p-5">
    <h4 className="font-bold">制御実験室 · Elevator</h4>
    <p className="mt-2 text-xs leading-6 text-[#657083]">0→0.6 mの目標を速度0.8 m/s・加速度1 m/s²で生成します。まずFFを切り、次に質量・遅延・ゲインを変えて、誤差と飽和を比較してください。</p>
    <div className="mt-4 grid gap-4 sm:grid-cols-2">{[
      ['kp','P [V/m]',0,100,1],['ki','I [V/(m·s)]',0,50,1],['kd','D [V·s/m]',0,30,1],['mass','実際の質量 [kg]',2,8,0.5],['voltageLimit','出力上限 [V]',4,12,0.5],
    ].map(([key,label,min,max,step]) => <label key={key} className="text-xs font-semibold">{label}：{parameters[key as keyof ControlParameters]}<input type="range" aria-label={String(label)} min={Number(min)} max={Number(max)} step={Number(step)} value={Number(parameters[key as keyof ControlParameters])} onChange={e => update(key as keyof ControlParameters, Number(e.target.value))} className="mt-2 w-full"/></label>)}</div>
    ...
    <svg viewBox="0 0 640 250" role="img" aria-label="6秒間の位置応答。青は実位置、緑の破線は目標位置。" className="mt-4 w-full">
      ...
      <text x="5" y="20" fontSize="12">位置 m</text><text x="5" y="40" fontSize="12">{maxPosition.toFixed(2)}</text><text x="30" y="225" fontSize="12">0</text><text x="555" y="235" fontSize="12">6秒</text>
    </svg>
    <p className="text-xs text-[#657083]">青：実位置 ／ 緑破線：目標</p>
    <div className="mt-3 grid gap-2 text-xs sm:grid-cols-3"><p>最終誤差：{((final.position-0.6)*1000).toFixed(1)} mm</p><p>最大位置：{peak.toFixed(3)} m</p><p>飽和時間：約{saturation.toFixed(2)} s</p></div>
    <p className="mt-3 text-xs">最後の0.5秒が±20 mm以内：{stableAtEnd ? 'はい' : 'いいえ'}（制作課題の3秒以内の整定判定はCSVで別途確認）</p>
    <div className="mt-4 flex gap-3"><button onClick={exportCsv} className="rounded-lg bg-[#0d1833] px-3 py-2 text-xs font-bold text-white">条件とログをCSV保存</button><button onClick={() => setParameters(defaultControlParameters)} className="rounded-lg border px-3 py-2 text-xs">初期条件に戻す</button></div>
    <p className="mt-4 text-xs leading-6 text-[#657083]">教育用モデル：m×加速度=8×電圧−3×速度−mg、刻み10 ms、床で停止。モータの電気・熱特性、バックラッシュ、上端衝突を含みません。実機のゲインや安全性を保証するモデルではありません。</p>
  </section>;
}
