...
function InlineFeedback({
  ...
  return <div className="mt-5 border-t border-[#eef0f4] pt-4">
    <div className="flex flex-wrap items-center gap-2"><span className="mr-1 text-[10px] font-bold text-[#a1a8b3]"><Text>{"この部分"}</Text></span><button disabled={sent} onClick={()=>{setRating('good');send('good')}} className={`inline-flex items-center gap-1 rounded-lg border px-2.5 py-1.5 text-xs font-bold ${rating==='good'?'border-[#1e7c3c] bg-[#e6f3ea] text-[#1e7c3c]':'border-[#d9dde5] text-[#657083]'}`}><ThumbsUp size={13}/>Good</button><button disabled={sent} onClick={()=>{setRating('bad');setOpen(true)}} className={`inline-flex items-center gap-1 rounded-lg border px-2.5 py-1.5 text-xs font-bold ${rating==='bad'?'border-[#c83833] bg-[#f9e9e8] text-[#c83833]':'border-[#d9dde5] text-[#657083]'}`}><ThumbsDown size={13}/>Bad</button><button onClick={()=>setOpen(v=>!v)} className="inline-flex items-center gap-1 rounded-lg border border-[#d9dde5] px-2.5 py-1.5 text-xs font-bold text-[#657083]"><MessageSquare size={13}/><Text>{"コメント"}</Text></button>{sent&&<span className="text-[10px] font-bold text-[#1e7c3c]"><Text>{"送信済み"}</Text></span>}</div>
    {open&&!sent&&<div className="mt-3 rounded-xl bg-[#f8f9fb] p-3"><select value={category} onChange={e=>setCategory(e.target.value)} className="w-full rounded-lg border border-[#d9dde5] bg-white p-2 text-xs"><option value="">{tr("カテゴリ（任意）")}</option>{feedbackCategories.map(c=><option key={c} value={c}><Text plain>{c}</Text></option>)}</select><textarea value={comment} onChange={e=>setComment(e.target.value)} rows={2} placeholder={tr("どこを直すと良さそう？")} className="mt-2 w-full resize-none rounded-lg border border-[#d9dde5] p-2 text-xs"/><button onClick={()=>send(rating)} disabled={!rating&&!comment.trim()} className="mt-2 rounded-lg bg-[#0d1833] px-3 py-2 text-xs font-bold text-white disabled:opacity-30"><Text>{"送信"}</Text></button></div>}
  </div>;
}
...
<button onClick={()=>onOpenLesson?.(`deep-${lesson.skillId}`)}><Text>{"応用教材"}</Text></button>
<button onClick={()=>onOpenFoundation?.(lesson.skillId)}><Text>{"わからないところを基礎ドリルで確認"}</Text></button><button onClick={()=>onOpenRecall?.(lesson.skillId)}><Text>{"説明を反復する →"}</Text></button>
</div><div className="border-b border-[#e1e4eb] bg-[#f8f9fb] p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="kicker"><Text>{skill.section}</Text> · <Text>{skill.nameJa}</Text></div><h2 className="mt-2 text-2xl font-bold"><Text>{lesson.title}</Text></h2><p className="mt-2 text-sm leading-6 text-[#657083]"><Text>{lesson.summary}</Text></p></div><div className="rounded-full border border-[#d9dde5] bg-white px-3 py-1.5 text-xs font-semibold text-[#657083]"><Text>{lesson.estimatedMinutes}</Text> min · rev.<Text>{lesson.revision}</Text></div></div>
      ...
      <div className="border-b border-[#e1e4eb] p-4">
        {skill.prerequisites.length > 0 && <div className="mb-3 flex flex-wrap items-center gap-2 text-xs"><span className="text-[#657083]"><Text>{"先に学ぶ："}</Text></span>{skill.prerequisites.map(id => <button key={id} disabled={!onOpenLesson} onClick={() => onOpenLesson?.(`eng-${id}`)} className="rounded-full border px-3 py-1.5 text-[#0857a2]"><Text>{skills.find(s => s.id === id)?.nameJa}</Text></button>)}</div>}
        <div className="flex flex-wrap gap-2">{lesson.steps.map((s, i) => <button key={s.id} onClick={() => setIndex(i)} className={`rounded-lg border px-3 py-2 text-xs ${index === i ? 'border-[#0857a2] bg-[#e6eff7]' : ''}`}><Text>{i+1}</Text>. <MathText>{s.title}</MathText></button>)}</div>
      </div>
      {!inCheckpoint && step && <div className="p-6 sm:p-8">
        ...
        {(!step.prompt || revealed) && <button onClick={nextStep} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#0d1833] px-5 py-3 text-sm font-bold text-white"><Text>{"次へ "}</Text><ChevronRight size={16}/></button>}
      </div>}

      {inCheckpoint && !finished && item && <div className="p-6 sm:p-8">
        <div className="kicker">Closed-book checkpoint · <Text>{checkpointIndex+1}</Text>/<Text>{checkpoint.length}</Text></div><div className="mt-2 text-xs font-semibold text-[#714086]"><Text>{item.competency}</Text> · Lv.<Text>{item.difficulty}</Text></div><h3 className="mt-5 text-xl font-bold leading-8"><MathText>{item.prompt}</MathText></h3>
        {item.format==='mcq'?<div className="mt-6 grid gap-3">{item.options?.map((o,i)=><button key={o} disabled={!!checkpointOutcome} onClick={()=>setChoice(i)} className={`rounded-xl border p-4 text-left text-sm ${choice===i?'border-[#0857a2] bg-[#e6eff7]':'border-[#e1e4eb] bg-white hover:bg-[#f8f9fb]'}`}><Text>{String.fromCharCode(65+i)}</Text>. <MathText>{o}</MathText></button>)}</div>:<input disabled={!!checkpointOutcome} value={numeric} onChange={e=>setNumeric(e.target.value)} inputMode="decimal" placeholder={tr("数値を入力")} className="mt-6 w-full rounded-xl border border-[#d9dde5] p-4 outline-none focus:border-[#0857a2]"/>}
        {!checkpointOutcome?<div className="mt-6 grid gap-2 sm:grid-cols-2"><button onClick={()=>submitCheckpoint(true)} className="rounded-xl border border-[#d9dde5] bg-white px-4 py-3 text-sm font-bold text-[#657083]"><Text>{"わからない"}</Text></button><button onClick={()=>submitCheckpoint(false)} disabled={item.format==='mcq'?choice===null:!numeric.trim()} className="rounded-xl bg-[#0d1833] px-4 py-3 text-sm font-bold text-white disabled:opacity-30"><Text>{"回答する"}</Text></button></div>:<><div className={`mt-5 rounded-xl p-4 text-sm ${checkpointOutcome==='correct'?'bg-[#e6f3ea] text-[#1e7c3c]':'bg-[#fff4dc] text-[#8b5b08]'}`}><div className="font-bold"><Text>{checkpointOutcome}</Text></div><div className="mt-1 leading-6"><MathText>{item.explanation}</MathText></div></div><InlineFeedback targetType="checkpoint" targetId={item.id} lesson={lesson} skillId={lesson.skillId} revision={item.revision} context={{result:checkpointOutcome,timeSpentSec:Math.max(1,Math.round((Date.now()-checkpointStartedAt)/1000))}}/><button onClick={nextCheckpoint} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#0d1833] px-4 py-3 text-sm font-bold text-white"><Text>{checkpointIndex+1>=checkpoint.length?'結果へ':'次の問題'}</Text> <ChevronRight size={16}/></button></>}
      </div>}

      {finished && <div className="p-6 sm:p-8"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#e6f3ea] text-[#1e7c3c]"><Check/></div><h3 className="mt-4 text-2xl font-bold">Lesson complete</h3><p className="mt-2 text-sm text-[#657083]">Checkpoint <Text>{checkpointCorrect}</Text>/<Text>{checkpoint.length}</Text><Text>{"。読了と実践は別です。復習で時間を空けた再現性を確かめ、実践ノートに成果物を残してください。"}</Text></p>
        ...
        <div className="mt-7 rounded-2xl border border-[#e1e4eb] p-5"><div className="font-bold"><Text>{"この教材どうだった？"}</Text></div><div className="mt-4 flex gap-2"><button onClick={()=>setRating('good')} className={`flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-bold ${rating==='good'?'border-[#1e7c3c] bg-[#e6f3ea] text-[#1e7c3c]':'border-[#d9dde5]'}`}><ThumbsUp size={16}/>Good</button><button onClick={()=>setRating('bad')} className={`flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-bold ${rating==='bad'?'border-[#c83833] bg-[#f9e9e8] text-[#c83833]':'border-[#d9dde5]'}`}><ThumbsDown size={16}/>Bad</button></div><select value={category} onChange={e=>setCategory(e.target.value)} className="mt-4 w-full rounded-xl border border-[#d9dde5] bg-white p-3 text-sm"><option value="">{tr("カテゴリ（任意）")}</option>{feedbackCategories.map(c=><option key={c} value={c}><Text plain>{c}</Text></option>)}</select><textarea value={comment} onChange={e=>setComment(e.target.value)} rows={3} placeholder={tr("コメント（任意）")} className="mt-3 w-full resize-none rounded-xl border border-[#d9dde5] p-3 text-sm"/><button onClick={sendFeedback} disabled={sent||(!rating&&!comment.trim())} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-[#0d1833] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-30"><MessageSquare size={15}/><Text>{sent?'送信済み':'フィードバック'}</Text></button>{feedbackCount>0&&<div className="mt-3 text-xs text-[#747d8c]"><Text>{"このLessonへのローカルfeedback: "}</Text><Text>{feedbackCount}</Text></div>}</div>
        {refs.length>0&&<div className="mt-7"><div className="kicker">Deep dive</div><div className="mt-3 grid gap-2">{refs.map(r=><a key={r!.id} href={r!.url} target="_blank" rel="noreferrer" className="rounded-xl border border-[#d9dde5] p-3 text-sm font-semibold text-[#0857a2]"><Text>{r!.provider}</Text> · <Text>{r!.title}</Text></a>)}</div></div>}
      </div>}
    </div>
  </div>;
}
...
function StepExercise({ step, onReveal }: { step: LessonStep; onReveal: () => void }) {
  ...
  return <div className="mt-7 rounded-2xl border border-[#d9dde5] bg-[#f8f9fb] p-5">
    ...
    {step.options ? <div className="mt-4 grid gap-2">{step.options.map((o, i) => <button key={o} disabled={revealed} onClick={() => setChoice(i)} className={`rounded-xl border p-3 text-left text-sm ${choice === i ? 'border-[#0857a2] bg-[#e6eff7]' : 'border-[#d9dde5] bg-white'}`}><MathText>{o}</MathText></button>)}</div>
      : numeric ? <input aria-label={tr("演習の数値回答")} disabled={revealed} value={written} onChange={e => setWritten(e.target.value)} inputMode="decimal" placeholder={tr("数値を入力")} className="mt-4 w-full rounded-xl border p-3"/>
      : <textarea aria-label={tr("自分の言葉で説明")} disabled={revealed} value={written} onChange={e => setWritten(e.target.value)} rows={3} placeholder={tr("判断とその根拠を書いてから解説を開く")} className="mt-4 w-full rounded-xl border p-3"/>}
    {!revealed ? <button disabled={!canReveal} onClick={() => { setRevealed(true); onReveal(); }} className="mt-4 rounded-xl bg-[#0d1833] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-30"><Text>{open ? '解説と比較する' : '答え合わせ'}</Text></button>
      : <div className="mt-4 rounded-xl bg-white p-4 text-sm leading-7"><div className="font-bold"><Text>{open ? '自分の説明と比較（自動採点なし）' : correct ? '正解' : 'もう一度整理'}</Text></div><MathText>{step.explanation}</MathText></div>}
  </div>;
}
