...
export function LessonPlayer({ lesson, onBack }: { lesson: Lesson; onBack: () => void }) {
  ...
  return <div className="mx-auto max-w-4xl space-y-5">
    ...
    <div className="panel overflow-hidden">
      <div className="border-b border-[#e1e4eb] bg-[#f8f9fb] p-5 sm:p-6">
      ...
      {inCheckpoint && !finished && item && <div className="p-6 sm:p-8">
        <div className="kicker">Closed-book checkpoint · {checkpointIndex+1}/{checkpoint.length}</div><div className="mt-2 text-xs font-semibold text-[#714086]">{item.competency} · Lv.{item.difficulty}</div><h3 className="mt-5 text-xl font-bold leading-8">{item.prompt}</h3>
        {item.format==='mcq'?<div className="mt-6 grid gap-3">{item.options?.map((o,i)=><button key={o} disabled={!!checkpointOutcome} onClick={()=>setChoice(i)} className={`rounded-xl border p-4 text-left text-sm ${choice===i?'border-[#0857a2] bg-[#e6eff7]':'border-[#e1e4eb] bg-white hover:bg-[#f8f9fb]'}`}>{String.fromCharCode(65+i)}. {o}</button>)}</div>:<input disabled={!!checkpointOutcome} value={numeric} onChange={e=>setNumeric(e.target.value)} inputMode="decimal" placeholder="数値を入力" className="mt-6 w-full rounded-xl border border-[#d9dde5] p-4 outline-none focus:border-[#0857a2]"/>}
